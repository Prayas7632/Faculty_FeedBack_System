...
{header('location:../home.php');}
	...
	{ echo "kmmlmk,";
      echo($email);
    $q=mysqli_query($conn,"select * from teacher where T_email='$email'");
	  $r=mysqli_num_rows($q);
       echo $r;
		if($r)
		{
			  $q1=mysqli_query($conn,"select * from teacher where T_email='$email' and T_sem = '$sem' and T_course = '$course'");
				$r1=mysqli_num_rows($q1);

				if($r1)
				{
					$err="<font color='blue' size='5px'><center>This faculty has already taken this course in this semester.</center></font>";
				}
				else
				{
					mysqli_query($conn,"insert into teacher ( `T_name`, `T_desg`,`T_course`, `T_sem`, `T_email`, `T_password`, `admin_id`) values ('$name','$Designation','$course','$sem','$email','$pass', 1)");
